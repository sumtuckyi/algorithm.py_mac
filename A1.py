...
def possible(v, group):  # 그룹의 연결여부 판단
    ...
    visited[v] = 1
    while q:
        start = q.popleft()  # deque를 import해서 leftpop()사용할 것
        for n in group:
            if adj_m[start-1][n-1] == 1 and visited[n] == 0:
                q.append(n)
                visited[n] = 1
    if visited.count(1) == len(group):
        return True
    return False
...
for tc in range(1, T + 1):
    ...
    for i in range(1, N//2 + 1):  # 2개의 권역으로 나누는 모든 경우의 수 탐색
        comb_generator = combinations([j for j in range(1, N+1)], i)
        for com in comb_generator:
            g1 = list(com)
            g2 = list(set([i for i in range(1, N+1)]) - set(g1))
